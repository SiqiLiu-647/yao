...
for (ii=1; ii <= numberof(gainv);ii++) {
  ...
  aoloop,disp=10;
  go, all=1;
  strehlarray(3,ii) = strehllp(0); // fill in result array
}
//------------------PYR Fixed------------------
aoread,"sh12x12_pyr_24.par";
sim.svipc=0;
...
for (ii=1;ii<=numberof(gainv);ii++) {
  ...
  aoloop,disp=10;
  go, all=1;
  strehlarray(4,ii) = strehllp(0); // fill in result array
}
//------------------Plotting------------------------------------------
window,1;
fma;
...
plg,strehlarray(3,),gsmagv,color=-9;
plmk,strehlarray(3,),gsmagv,color=-9,marker=5;
plg,strehlarray(4,),gsmagv,color=-11;
plmk,strehlarray(5,),gsmagv,color=-11,marker=7;
limits, 0.5, 16.5;//x-axis
range, 0.0, 0.75;//y-axis

logxy,0,0;
xytitles,"Magnitude",swrite(format="Strehl @ %.2fmicrons",(*target.lambda)(0));
pltitle, "Strehl Ratio with the magnitude in K band";
